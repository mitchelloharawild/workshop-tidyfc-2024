...
```{r arp-pacf, echo=FALSE, fig.height=2.3}
set.seed(1)
p1 <- tsibble(idx = seq_len(100), sim = 10 + arima.sim(list(ar = -0.8), n = 100), index = idx) |>
  PACF(sim) |> 
  autoplot() + labs(title = "AR(1)")
p2 <- tsibble(idx = seq_len(100), sim = 20 + arima.sim(list(ar = c(1.3, -0.7)), n = 100), index = idx) |>
  PACF(sim) |> 
  autoplot() + labs(title = "AR(2)")
gridExtra::grid.arrange(p1, p2, nrow = 1)
```
...
```{r maq-acf, fig.height=2.3, echo=FALSE}
set.seed(2)
p1 <- tsibble(idx = seq_len(100), sim = 20 + arima.sim(list(ma = 0.8), n = 100), index = idx) |>
  ACF(sim) |> 
  autoplot() + labs(title = "MA(1)")
p2 <- tsibble(idx = seq_len(100), sim = arima.sim(list(ma = c(-1, +0.8)), n = 100), index = idx) |>
  ACF(sim) |> 
  autoplot() + labs(title = "MA(2)")
...
## Cortecosteroid medication subsidies

```{r, echo=TRUE}
cortecosteroid_subsidies <- PBS |>
  filter(ATC2 == "H02") |>
...
```{r, echo=TRUE}
cortecosteroid_subsidies |> autoplot(
  Cost
)
```
...
fit |>
  forecast() |>
  autoplot(cortecosteroid_subsidies) +
  labs(y = "Cortecosteroid Expenditure ($AUD)", x = "Year")
```
...
Or, combine them with ensembles!
...
```{r trainall2, echo=TRUE, dependson='tourism'}
train <- student_labour |>
  filter(month <= yearmonth("2021 Aug"))
fit <- train |>
  model(
    ets = ETS(persons),
    arima = ARIMA(persons),
...
```{r trainfc2, dependson='trainall'}
#| fig-height: 2.5
fc <- fit |> forecast(h = "3 years")
fc |>
  filter(
    state == "Australian Capital Territory",
    attendance == "Attending full-time education",
    status == "Employed full-time"
  ) |>
  autoplot(student_labour |> filter(month >= yearmonth("2010 Jan")), level = NULL)
```

## Forecast ensembles
\fontsize{10}{11}\sf

```{r ensemble-acc-2, dependson='trainfc'}
accuracy(fc, student_labour) |>
  group_by(.model) |>
  summarise(
    RMSE = mean(RMSE),
    MAE = mean(MAE),
