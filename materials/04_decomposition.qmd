...
## Tertiary education payroll
\fontsize{11}{11}\sf

```{r tertiary}
tertiary_payroll <- payroll_education |>
  filter(Industry == "Tertiary Education")
tertiary_payroll
```
...
```{r tertiary-stl, fig.width=8, fig.height=4}
...
```{r tertiary3}
components(dcmp) |> gg_season(season_year)
```
...
tertiary_payroll |>
  model(STL(Jobs ~ trend(window = 15) + season(window = "periodic"),
    robust = TRUE
  )) |>
  components()
```
...
\fontsize{12}{13}\sf
  ...
  *  `trend(window = ?)` controls variation of trend component.
...
* Smooths trend and seasonal components iteratively.
...
vic_elec |>
  ...
  components() |>
  autoplot()
crop::dev.off.crop("figs/vic_elec_stl.png")
```
