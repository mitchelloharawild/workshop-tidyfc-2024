...
Data from the ABS often mixes aggregate values into the data.

This is done inconsistently ("Total", "AUST", etc.) and isn't tidy.

To work with aggregates in tidy data analysis, we prefer to compute them ourselves with `group_by()` and `summarise()`. 
:::
